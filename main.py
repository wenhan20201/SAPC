...
from config_SPAC import options
from Dataset_SPAC import VideoDataset
from utils import getClassificationMAP as cmAP, ANETdetection
from model_SPAC import SPAC
...
def train_one_epoch(epoch, args, dataset, model, optimizer, logger, device):
    ...
    for step, sample in enumerate(train_loader):
        ...
        loss_dict = model.criterion(outputs, labels, proposals, epoch=epoch, args=args, device = device)
    ...
    for key in loss_dict_sum.keys():
        ...
        # 取出当前 epoch 的平均伪标签熵
    epoch_pseudo_entropy = loss_dict_sum['pseudo_entropy'] / len(train_loader)
    return epoch_pseudo_entropy
...
def train(args, dataset, model, device):
    ...
    if os.path.exists(os.path.join(checkpoint_dir, 'best_model_SPAC.pkl')):
       os.remove(os.path.join(checkpoint_dir, 'best_model_SPAC.pkl'))

    max_map = np.zeros(7) if 'Thumos' in args.dataset_name else np.zeros(10)
    pseudo_entropy_history = []
    for epoch in tqdm(range(1, args.max_epoch+1)):
        epoch_pseudo_entropy =train_one_epoch(epoch, args, dataset, model, optimizer, logger, device)
        pseudo_entropy_history.append(epoch_pseudo_entropy)
        ...
        if epoch % args.interval == 0:
            ...
            if cond:
                torch.save(model.state_dict(), os.path.join(checkpoint_dir, 'best_model_SPAC.pkl'))
                max_map = dmap
    np.save(os.path.join(args.exp_dir, 'pseudo_entropy.npy'), np.array(pseudo_entropy_history))
    model.load_state_dict(torch.load(os.path.join(checkpoint_dir, 'best_model_SPAC.pkl')))
...
def test(epoch, args, dataset, model, logger, device):
    ...
    for step, sample in enumerate(test_loader):
        ...
        prediction, pred, pred_vid_score = utils.get_prediction_fused(videoname, outputs, dataset, args)
...
if __name__ == '__main__':
    ...
    appearance_descriptors = torch.from_numpy(dataset.get_appearance_descriptors()).float().to(device)
    motion_descriptors = torch.from_numpy(dataset.get_motion_descriptors()).float().to(device)

    model =SPAC(args, appearance_descriptors, motion_descriptors).to(device) # (args, appearance_descriptors, motion_descriptors)
    if args.run_type != 'train' and args.pretrained_ckpt is not None:
        try:
                # 尝试直接加载
            model.load_state_dict(torch.load(args.pretrained_ckpt))
        except RuntimeError as e:
            if "Unexpected key(s) in state_dict" in str(e):
                print("⚠️  Detected teacher model keys in checkpoint, filtering them out...")
                    # 加载检查点并过滤掉教师模型相关的键
                checkpoint = torch.load(args.pretrained_ckpt)
                filtered_checkpoint = {k: v for k, v in checkpoint.items() if not k.startswith('teacher_model.')}
                model.load_state_dict(filtered_checkpoint, strict=False)
                print("✅ Model loaded successfully after filtering teacher model keys")
            else:
                raise e  # 重新抛出其他错误    
    # 初始化教师模型
